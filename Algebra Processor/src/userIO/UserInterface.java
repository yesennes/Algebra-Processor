...
public class UserInterface extends Application implements Serializable {
    ...
    public void start(Stage stage) {
        AnchorPane topAnchor = new AnchorPane(), bottomAnchor
                = new AnchorPane();
        BorderPane main = new BorderPane();
        VBox topmost = new VBox();
        HBox top = new HBox(5), other = new HBox(5);

        output = new TextArea("Enter an equation in the box above, or click on "
                + "the \"?\" in the bottom right for more help.");
        ...
        Label descrip = new Label("All non-rational numbers will be kept in as "
                + "mathmatical constants and exponents.");
            ...
            view.setOnAction(s -> {
                if (matrices.size() == 0) {
                    ...
                    for (Map.Entry<String, Matrix> entry
                        ...
                        display.appendText(entry.getKey() + " " + m.row + " x "
                                + m.col + "\n" + m + "\n");
                    }
                    viewing.setScene(new Scene(display));
                    viewing.show();
                }
            });
        ...
        */
        ...
        ToolBar buttons = new ToolBar(i, pi, bE, matrix); //, not, and, or, bIf, iff);
